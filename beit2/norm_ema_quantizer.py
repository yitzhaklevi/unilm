...
class EmbeddingEMA(nn.Module):
    ...
    def weight_update(self, num_tokens):
        ...
        self.weight.data.copy_(embed_normalized, non_blocking=True)


def norm_ema_inplace(moving_avg, new, decay):
    moving_avg.data.mul_(decay).add_(new, alpha=(1 - decay))
    moving_avg.data.copy_(l2norm(moving_avg.data), non_blocking=True)
...
class NormEMAVectorQuantizer(nn.Module):
    def __init__(self, n_embed, embedding_dim, beta, decay=0.99, eps=1e-5,
        ...
        if distributed.is_available() and distributed.is_initialized():
            ...
            self.all_reduce_fn = lambda pred: lambda tensor: distributed.all_reduce(tensor, async_op=pred)(True)
        else:
    ...
    def reset_cluster_size(self, device):
        if self.statistic_code_usage:
            ...
            self.cluster_size = self.cluster_size.to(device, non_blocking=True)
